...
| **Prompt tái dùng** | `docs/ai/prompts/{review,review-vs-plan,generate-api-docs,generate-test,refactor,generate-feature,commit-message,find-reuse,task-breakdown}.md` | Logic cho review diff / đối chiếu code với Plan / docs FE / test / refactor / feature / commit / reuse / bẻ việc-estimate |
...
Ý nghĩa lệnh: `/review` (soát diff theo checklist) · `/review-vs-plan` (dùng AI/phiên độc lập đối chiếu implementation với Plan cuối) · `/refactor` (review/refactor giữ behavior, mức 🔴🟡🟢) · `/api-docs` (sinh docs FE `api-docs/<Module>/<Endpoint>.md`) · `/scaffold-test` (sinh unit test Mockery) · `/task-breakdown` (bẻ việc/bóc task/estimate theo Size×Effort→Point, mỗi task ≤ 2 Point).
...
- Gõ `/` xem có `review`/`review-vs-plan`/`refactor`/`api-docs`/`scaffold-test`/`scaffold-feature`/`commit-message`/`find-reuse`/`task-breakdown` không.
