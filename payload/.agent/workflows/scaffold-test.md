...
Bắt buộc bám code thật: đọc class + các interface dependency để mock đúng method/đối số; phủ happy-path + từng nhánh `BusinessException`. Ghi vào `source/tests/Unit/<ClassName>Test.php` theo AAA, tên `test_<scenario>_<outcome>`. Kiểm chứng bằng Docker: `make ai-test TEST=tests/Unit/<ClassName>Test.php`.
