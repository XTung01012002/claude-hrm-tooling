...
Bắt buộc: review diff TRƯỚC → verdict (`PASS` / `PASS_WITH_CONCERNS` / `REQUEST_CHANGES` / `BLOCKED_INSUFFICIENT_CONTEXT`) → rồi mới sinh branch + commit + PR summary. Chỉ đề xuất branch/commit khi verdict là `PASS` hoặc `PASS_WITH_CONCERNS` và không có bất kỳ finding nào được đánh dấu `Merge blocking: Yes`.
