...
```
Assume the implementation contains at least one subtle bug.
...
> ⚠️ `git diff` **không thấy file untracked** (file mới chưa `git add`). Luôn chạy `git status --short --untracked-files=all` trước và đọc cả file untracked liên quan. Nếu có nhiều untracked, yêu cầu user `git add -A` trước.
...
- **[file.php:line]** <vấn đề>
  - Tình huống tái hiện: <input/điều kiện cụ thể>
  - Hậu quả: <gì sẽ xảy ra>
  - Bằng chứng: <grep/đọc code xác nhận>
