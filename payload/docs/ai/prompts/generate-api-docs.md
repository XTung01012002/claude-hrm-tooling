...
5. **Handler và nhánh lỗi có thể đi tới** (`<Feature>Handler.php`): lấy shape return; liệt kê `BusinessException(message, httpCode)` và exception HTTP cụ thể trong Handler. Trace các class/method mà Handler gọi cho đến khi gặp một boundary rõ ràng: exception được catch/translate, đã tới Infrastructure/external client boundary, đã xác định exception có thể bubble ra HTTP renderer, hoặc phát hiện cycle. Nếu gọi qua interface thì tìm implementation qua binding. Chỉ ghi lỗi có nhánh code đi tới được trong phạm vi trace; nếu dừng do giới hạn context, ghi rõ lỗi sâu hơn chưa được verify.
