...
2. **Đường dẫn đầy đủ** = prefix `api/v1` (khai ở `source/config/route-attributes.php`) + path trong attribute. Verify bằng Docker: `make -f Makefile.ai ai-artisan CMD="route:list --path=<...>"`. KHÔNG tự ghép/đoán path.
