...
>
> **Đọc trước:** `docs/ai/PROJECT-CONVENTIONS.md` — đây là nguồn chân lý hiện hành của HRM API. Không dùng trí nhớ hoặc snapshot cũ thay cho code/config thật.
...
3. Nếu chỉ có repository hiện tại mà chưa nêu range:
   ...
   - Chạy `git status --short` và đọc file untracked thuộc phạm vi vì `git diff` không hiển thị chúng.
...
| Mức | Khi dùng |
|---|---|
| 🔴 Cao | Lỗi production/security, sai hoặc mất dữ liệu, rò rỉ tenant, phá public contract hoặc bỏ sót yêu cầu cốt lõi; phải chứng minh đường gây lỗi và tác động |
| 🟡 Trung bình | Sai acceptance criteria, regression/edge case/hiệu năng/maintainability có tác động cụ thể nhưng chưa tới mức Cao |
| 🟢 Thấp / Cần xác nhận | Vi phạm convention nhỏ, readability/cleanup hoặc rủi ro hợp lý nhưng còn thiếu dữ kiện |
| 🔵 Gợi ý | Cải tiến không bắt buộc, không ảnh hưởng kết luận merge |

Không gắn mức độ chỉ vì loại lỗi xuất hiện trong checklist. Ví dụ thiếu transaction hoặc N+1 chỉ được nâng mức khi đã verify luồng và tác động thực tế. Không chắc chắn thì không gắn mức Cao.

## 6. Kết luận merge

- `❌ Không đạt`: còn sai lệch Plan hoặc finding Cao/Trung bình đủ sức chặn merge.
- `⚠️ Đạt có điều kiện`: implementation cơ bản đúng nhưng còn điểm cần xác nhận/sửa trước hoặc ngay sau merge; phải ghi điều kiện cụ thể.
- `✅ Đạt`: mọi mục Plan đã được verify, không có finding chặn merge và giới hạn kiểm chứng không làm thay đổi kết luận.

Không đề xuất tính năng mới trong kết luận. Ý tưởng cải tiến để riêng ở mục gợi ý.
...
- 🟡 [Pxx] [Plan] `file.php:line` — vấn đề → tác động → bằng chứng → hướng sửa
...
- 🔴/🟡/🟢 [Pxx hoặc N/A] [Chất lượng/Convention/Security/Performance] `file.php:line` — vấn đề → tác động → bằng chứng → hướng sửa
...
✅ Đạt / ⚠️ Đạt có điều kiện / ❌ Không đạt

<2–3 câu nêu lý do và điều kiện cần xử lý trước khi merge.>

### 8. Gợi ý ngoài phạm vi, không bắt buộc
