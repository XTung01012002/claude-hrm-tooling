...
>
> **Đọc trước:** `docs/ai/PROJECT-CONVENTIONS.md` (đặc biệt §0 cấm bịa, §1 reuse+DRY, §2 layering, §4 ORM, §5 bẫy, §8 multi-tenancy, §9 transaction/webhook, §10 list, §11 giữ behavior) và `docs/ai/prompts/_shared/review-contract.md`.
...
3. Kiểm tra comment, context nghiệp vụ, schema và pattern hiện có trước khi flag. Quy tắc này áp dụng cho **mọi** checklist, không chỉ ngoại lệ layering. Nếu code có ngoại lệ chủ đích và vẫn an toàn thì không flag; nếu lý do chưa đủ rõ thì xếp `QUESTION` và nêu điều cần xác nhận.
...
5. Không gắn mức độ chỉ vì mục đó xuất hiện trong checklist. Vi phạm convention thuần túy như thiếu `strict_types` hoặc dùng magic string mặc định là **SUGGESTION**, trừ khi có bằng chứng cụ thể rằng nó gây bug hoặc rủi ro lớn hơn.
...
## Xếp mức độ và verdict (thống nhất toàn project)

Áp dụng contract chung ở `docs/ai/prompts/_shared/review-contract.md`; không tự tạo taxonomy khác.
...
- **❓ QUESTION:** rủi ro chưa đủ dữ kiện xác nhận, cần author trả lời.

Không chắc chắn thì không được gắn **BLOCKER**. Nêu chính xác điều đã verify và phần nào còn cần xác nhận.
...
Tổng kết: `N` blocker, `N` important, `N` suggestion, `N` question. Ngoài phạm vi: `N`.
```
...
- Finding **BLOCKER** và **IMPORTANT** phải có đầy đủ schema (Severity → Required test). **SUGGESTION** và **QUESTION** có thể rút gọn.
